...
f.e1 <- function(PARA=1, STUD=my.stud, DEAT=my.deat, VDT=my.vdt, 
                 VXF=my.vxf, VXR=my.vxr, MMM=my.mmm, MODL=my.modl, LAMB=my.lamb, 
  ...
  ##  MMM  = Method Misclassification Matrix
  ##  -------------- #
  ...
  jd.r1 <- list(S = S, C = C, K = K, rN = rN, lambda = LAMB, 
                ...
                rG = STUD$rG, XM = XM, GM = GM, MMM=MMM)
  ...
  para <- list(name = NAME, time = ptime, PARA = PARA, 
               ...
               LAMB = LAMB, RSDL = RSDL, QUAD = QUAD, MMM=MMM)
  ...
  if(SAMC==F) jm.r1$BUGSoutput$sims.array <- NULL
